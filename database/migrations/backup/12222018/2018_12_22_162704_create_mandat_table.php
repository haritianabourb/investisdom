...
class CreateMandatTable extends Migration {
	...
	public function up()
	{
		Schema::create('mandat', function(Blueprint $table)
		{
			...
			$table->float('montant_ht', 10, 0)->default('0::double precision');
			$table->float('carte_grise', 10, 0)->default('0::double precision');
			$table->float('fraix_defiscalisable', 10, 0)->default('0::double precision');
			$table->float('fraix_non_defiscalisable', 10, 0)->default('0::double precision');
			$table->float('tva_npr', 10, 0)->default('0::double precision');
			$table->float('tva_investissement', 10, 0)->default('0::double precision');
			$table->boolean('is_remplacement')->default(0);
			$table->float('montant_remplacement', 10, 0)->nullable()->default('0::double precision');
			$table->boolean('is_assurance')->default(0);
			$table->boolean('is_reprise_fournisseur')->default(0);
			$table->float('montant_reprise_fournisseur', 10, 0)->nullable()->default('0::double precision');
			$table->date('prevision_livraison')->nullable();
			$table->float('apport_locataire', 10, 0)->default('0::double precision');
			$table->float('apport_snc', 10, 0)->default('0::double precision');
			$table->float('apport_bd', 10, 0)->default('0::double precision');
			$table->float('apport_investissement', 10, 0)->default('0::double precision');
			$table->boolean('is_subvention')->default(0);
			$table->string('type_subvention')->nullable()->default('');
			$table->float('montant_frais_tenue_compte', 10, 0)->default('75::double precision');
			$table->integer('nombre_periode')->default(1);
			$table->string('periodicite')->default('MENSUELLE');
			$table->integer('duree_pret')->default(60);
			$table->float('taux_pret', 10, 0)->default('5.6::double precision');
