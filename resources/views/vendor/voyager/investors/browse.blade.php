...
@section('page_header')
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-9">
                <h1 class="page-title">
                    <i class="{{ $dataType->icon }}"></i> {{ __('voyager::generic.viewing') }} {{ ucfirst($dataType->display_name_plural) }}
                    &nbsp;
                </h1>
            </div>
            <div class="col-md-3">
                            ...
                            <table id="dataTable" class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    @foreach($dataType->browseRows as $row)
                                        @if($row->field === "investor_belongsto_cgp_relationship" && !\Auth::user()->hasRole(['admin', 'investis', 'investisdom']))
                                            @php
                                                continue;
                                            @endphp
                                        @endif
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            <td>
                                                    ...
                                                    @if(property_exists($row->details, 'relationship'))
                                                        @foreach($data->{$row->field} as $item)
                                                            @if($item->{$row->field . '_page_slug'})
                                                                <a href="{{ $item->{$row->field . '_page_slug'} }}">{{ $item->{$row->field} }}</a>
                                                                @if(!$loop->last)
                                                                    ,
                                                                @endif
                                                            @else
                                                ...
                                                @elseif($row->type == 'select_dropdown' && property_exists($options, 'options'))
                                                    ...
                                                    @else
                                                        {!! $options->options->{$data->{$row->field}} ?? __('voyager::generic.none') !!}
                                                    @endif
